...
asm ("movia r2, main");  // C 언어의 main program이 있는 주소를 불러와
asm ("jmp r2"); // Jump 한다.
}
...
asm ( "subi ea, ea, 4");  /* 외부 interrupt는 리턴 할 주소를 한 명령어 앞당겨야 한다.
* interrupts 인경우 아래의 명령어를 수행한다.*/
asm ( "SKIP_EA_DEC:" );
asm ( "stw r1, 4(sp)" );  // 모든 register를 Stack에 저장
asm ( "stw r2, 8(sp)" );
asm ( "stw r3, 12(sp)" );
asm ( "stw r4, 16(sp)" );
asm ( "stw r5, 20(sp)" );
...
pushbutton_ISR( );  // pushbuttons interrupt인경우 pushbutton ISR을 호출한다.
